# Contributing to Noxaria Checker

Thank you for considering contributing to Noxaria Checker! This document provides guidelines and instructions for contributing.
...
### 1. Fork and Clone
```bash
git clone https://github.com/YOUR_USERNAME/Noxaria-Checker.git
cd Noxaria-Checker
git remote add upstream https://github.com/DarkFlux4T/Noxaria-Checker.git
```
...
4. **Test Your Gate**
   ...
   >>> from gates.your_gate import your_gate
   >>> import asyncio
   >>> result = asyncio.run(your_gate("4242424242424242", "12", "2026", "123"))
   >>> print(result)
   ```
...
async def test_your_gate_approved():
    status, msg = await your_gate("4242424242424242", "12", "2026", "123")
    assert status.startswith("Approved")

@pytest.mark.asyncio
async def test_your_gate_invalid():
    status, msg = await your_gate("invalid", "12", "2026", "123")
...
**Thank you for contributing to Noxaria Checker! 🎉**
